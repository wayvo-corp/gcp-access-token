...
pub async fn init(sub: String, file_path: String, scope: String) -> Result<String, &'static str> {
    //--------------------------------------
    //process creds
    //--------------------------------------
    ...
    if !creds.has_key(&"private_key") {
        return Err("not_found-creds-private_key");
    }
    match creds["private_key"].as_str() {
        Some(v) => {
            private_key_string = v.to_string();
        }
        None => {
            return Err("not_found-creds-private_key");
        }
    ...
    let email: String;
    if !creds.has_key(&"client_email") {
        return Err("not_found-creds-client_email");
    }
...
fn sign_token(data: Vec<u8>, key: PKey<Private>) -> Result<String, &'static str> {
    ...
    match signer.update(&data) {
        Ok(_) => {}
        Err(_) => {
            return Err("failed-update-signer");
        }
    }
